...
import { Badge, Button, Card, CardHeader, EmptyState, Input, ListRow, PasskeyPrompt, Text, useToast } from "@/components/ui";
...
export function AllowListCard() {
  ...
  const toggle = (v: boolean) => run(`Allow-list ${v ? "enabled" : "disabled"}`, () => setEnabled(v));

  const add = () => {
    const a = draft.trim().toUpperCase();
    if (!isContractAddress(a)) return toast.error("Invalid address", "Enter a 56-character contract address (C…).");
  ...
  const remove = (a: string) => run("Address removed", () => setAddresses((p) => p.filter((x) => x !== a)));
  ...
  return (
    <Card>
      <CardHeader
        ...
        action={
          <View style={[styles.toggle, { backgroundColor: colors.muted, borderColor: colors.border, borderRadius: radius.lg }]}>
      ...
      {sign.isPending ? (
        ...
        <View style={[styles.note, { backgroundColor: colors.muted, borderColor: colors.border, borderRadius: radius.lg }]}>
          <Ionicons name="shield-outline" size={18} color={colors.mutedForeground} />
          <Text variant="small" tone="muted" style={{ flex: 1 }}>
            Allow-list is off. Your wallet can interact with any contract. Turn it on to restrict access.
          </Text>
        </View>
      ) : (
        <View style={{ gap: 14 }}>
          <View style={[styles.note, { backgroundColor: alpha(colors.success, 0.08), borderColor: alpha(colors.success, 0.3), borderRadius: radius.lg }]}>
          ...
          <Input
            ...
            onSubmitEditing={add}
            right={
              <Button size="sm" disabled={!draft.trim()} onPress={add} icon={<Ionicons name="add" size={14} color={colors.primaryForeground} />}>
                Add
              </Button>
            }
          />
          {addresses.length === 0 ? (
            <EmptyState title="No addresses allowed yet" description="All transactions are blocked." />
          ) : (
                ...
                <ListRow
                  ...
                  right={
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                      ...
                      <Button size="sm" variant="ghost" haptic={false} onPress={() => remove(a)} accessibilityLabel="Remove">
...
const styles = StyleSheet.create({
  toggle: { flexDirection: "row", alignItems: "center", gap: 8, paddingLeft: 12, paddingRight: 6, paddingVertical: 4, borderWidth: 1 },
